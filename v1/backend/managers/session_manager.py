...
class SessionManager:
    ...
    def _create_chat_record(self, chat_id: str, user_id: str, profile_id: str):
        ...
        try:
            query = """
            INSERT INTO chat_history (chat_id, user_id, profile_id, title)
            VALUES (%s, %s, %s, %s)
            """
            
            # Create a default title
            title = f"Chat for profile {profile_id}"
            
            self.db.execute_query(query, (chat_id, user_id, profile_id, title))
            logger.debug(f"Chat record created: {chat_id}")
    ...
    def _store_chat_message(self, chat_id: str, message: Dict):
        ...
        try:
            query = """
            INSERT INTO chat_messages (message_id, chat_id, message_type, content)
            VALUES (%s, %s, %s, %s)
            """
            ...
            self.db.execute_query(query, (message_id, chat_id, message_type, content))
            logger.debug(f"Message stored in database: {message_id}")
